...
const DragonCursor = () => {
  ...
  useEffect(() => {
    ...
    const initialElems = [];
    for (let i = 0; i < N; i++) {
        initialElems[i] = { use: null, x: width / 2, y: 0 };
    }
    elemsRef.current = initialElems;
    ...
    const radm = Math.min(pointerRef.current.x, pointerRef.current.y) - 20;

    let requestID: number;
    const run = () => {
      requestID = requestAnimationFrame(run);
      let e0 = elemsRef.current[0];
      ...
      e0.x += (ax + pointerRef.current.x - e0.x) / 10;
      e0.y += (ay + pointerRef.current.y - e0.y) / 10;
      ...
      if (radRef.current < radm) radRef.current++;
